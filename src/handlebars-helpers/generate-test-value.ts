...
export const generateCSharpTestValue = (typeName: string): string => {
    const targetType = allTypes.find(t => t.isType(typeName))
    if (!targetType) {
        throw new Error(`'${typeName}' is an undefined argument type.`)
    }
    if (targetType === listArgument) {
        throw new Error('Generating C# test values for List type arguments is not supported.')
    }
